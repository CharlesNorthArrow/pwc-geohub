...
// 10 MB upload ceiling — pwc_schools.csv is ~2 MB today, headroom for growth.
const MAX_BYTES = 10 * 1024 * 1024;

/**
 * Multipart upload: `file` (CSV). Returns an upload_id + the column
 * classification. The CSV text is held server-side in an in-memory session
 * keyed by upload_id; preview/apply re-read from there so the client never
 * has to re-send the rows.
 *
 * The server re-runs `classifyColumns` independently of anything the client
 * computes — the dialog's job is to collect decisions, never to dictate the
 * classification.
 */
export async function POST(req: NextRequest): Promise<NextResponse> {
  return guardAdmin(async () => {
    ...
    try {
      ...
      // csv-parse returns objects keyed by header in column order; reconstruct.
    ...
    const classification = classifyColumns(headers);
