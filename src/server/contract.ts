...
function toPublic(i: IndicatorRegistryEntry): IndicatorPublic {
  return {
    ...
    scale: {
      ...
      layer_opacity: i.scale.layer_opacity,
    },
...
export async function getPwcProgram(dbn: string, year: string): Promise<PwcProgramResponse> {
  ...
  if (rows.length === 0) {
    return {
      program: {
        ...
        cohort: null,
        active: false,
        social_work_program: null,
        community_school_program: null,
        arts_program: null,
        ost_program: null,
  ...
  const program: PwcProgram = {
    ...
    cohort: r.cohort,
    active,
    social_work_program: r.social_work_program,
    community_school_program: r.community_school_program,
    arts_program: r.arts_program,
    ost_program: r.ost_program,
